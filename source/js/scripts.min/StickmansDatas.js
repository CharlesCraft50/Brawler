  ...
  game.physics.arcade.overlap(this, player, function(obj1, obj2){
    if(obj2.counter == 0) {
      ...
      if(obj2.randomNumber > 18 && obj2.randomNumber < 20) {
        ...
        if(obj2.holding == 'nothing') {
          new Weapons('uzi', true, obj2, true, 400, 20);
        }
  ...
  game.physics.arcade.overlap(this, enemies, function(obj1, obj2){
    if(obj2.counter == 0) {
      ...
      if(obj2.randomNumber > 18 && obj2.randomNumber < 20) {
        ...
        if(obj2.holding == 'nothing') {
          new Weapons('uzi', true, obj2, false, 400, 20);
        }
      } else if(obj2.randomNumber > 21 && obj2.randomNumber < 28) {
        if(obj2.holding == 'nothing') {
...
GameState.prototype.createStuff_Block = function(x, y) {
...
var createWeapon = function (weapon_name, speed, game, target, ammo) {
  ...
  if(this.weapon_name == 'pistol') {
    ...
    this.target.bullet = game.add.weapon(this.ammo, 'bullet_1');
    ...
    this.target.bullet = game.add.weapon(this.ammo, 'orb_magical_orange');
    this.target.weaponTime = 1000;
  ...
  if(this.target.alive == true) {
    ...
    this.target.bullet.fireLimit = this.ammo;
    }
  this.alive = true;
...
createWeapon.prototype.update = function () {
  //console.log(this.target.bullet.bullets.total);
  if(this.target.alive == true) {
    if(this.alive == true) {
      //collideWithTilemap(true, this);
      if(this.weapon_name) {
  ...
  if(this.target.weaponAmmo == 0) {
    this.visible = false;
    this.target.bodyAnimation.playAnimationByName('idle');
    if(this.target.bullet.bullets.total == 0) {
      this.target.counter = 0;
      this.target.holding = 'nothing';
      this.destroy();
      this.alive = false;
...
//Enemy_1
var Enemy_1 = function (game, player, health, x, y, dummy) {
...
if(getPowDistance(false, true, 0, this.body.y, 0, player.body.y) > 25600) {
  this.jump();
}
...
if(this.facing == 'left' && this.body.x < player.body.x) {
  ...
  if(this.enemyMoves != false) {
      ...
      if(game.physics.arcade.distanceBetween(player, this) < 200) {
...
collideWithTilemap(true, this);
game.physics.arcade.collide(this, borderBottom, function(){this.body.y = 450;}.bind(this));
...
if(this.AnimationUpdate == true) {
       ...
       this.breathingStartCounter = 0;
        ...
        if(this.breathingStartCounter == 0) {
          setTimeout(function(){this.breathingTime = 4000;}, 1000);
          this.bodyAnimation.updateAnimation();
          this.bodyAnimation.setAnimationSpeedPercent(100);
          if(this.holding != 'nothing' && this.holding != 'powers') {
            this.bodyAnimation.playAnimationByName('idle_hold');
  ...
  if(this.dummy == true) {
      this.bodyAnimation.updateAnimation();
      this.bodyAnimation.setAnimationSpeedPercent(100);
    if(game.time.time - this.timeCheckBreathing >= 4000) {
      if(this.holding != 'nothing' && this.holding != 'powers') {
        this.bodyAnimation.playAnimationByName('idle_hold');
      } else {
        this.bodyAnimation.playAnimationByName('idle');
...
GameState.prototype.createEnemy_1 = function(x, y) {
...
GameState.prototype.createDummy = function(x, y) {
    ...
    if (enemy) {
        ...
        enemy = new Enemy_1(this.game, player, health, x, y, true);
        enemies.add(enemy);
        if(health > 50) {
          //enemy.scale.setTo(1.2, 1.2);
        }
...
GameState.prototype.createNPC = function(x, y, spriterData, speed, moves, direction) {
...
var Player = function(game, x, y, spriterData) {
    ...
    player.breathingStartCounter = 0;
    player.deathStartCounter = 0;
    ...
    player.breathingTime = 1000;
    player.dead = false;

    player.coins = game.add.bitmapText(game.width-150, game.height-580, 'BatmanForeverOutline', 'Coins: ' + localStorage.getItem('score'), 20);
  ...
  new Weapons('uzi', true, player, true, 400, 20);
};
...
Player.prototype.update = function() {
  ...
  if(player.coinsCounter == 0) {
      ...
      game.physics.arcade.collide(player, borderBottom, function(){player.body.y = 450;});
      ...
      if(player.AnimationUpdate == true) {
       ...
       player.breathing = false;
        ...
        if(game.time.time - player.timeCheckBreathing >= player.breathingTime && player.doubleTapped == false) {
        ...
        if(player.breathingStartCounter == 0) {
          setTimeout(function(){player.breathingTime = 4000;}, 1000);
          player.bodyAnimation.updateAnimation();
          player.bodyAnimation.setAnimationSpeedPercent(100);
          if(player.holding != 'nothing' && player.holding != 'powers') {
            player.bodyAnimation.playAnimationByName('idle_hold');
